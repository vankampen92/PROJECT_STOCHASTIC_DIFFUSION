...
for(kk in 1:NREP){
  ...
  for(jj in 1:NEV){
    ...
    p_m_R+p_m_A+p_i_R+p_i_A+p_d_R+p_d_A+p_e+p_d
    ...
    if((pp>=p_i_R)&&(pp<p_i_R+p_i_A)){
      spatial_grid_A[rand_row,rand_col]<- spatial_grid_A[rand_row,rand_col]+1
    }
    #### death of resources
    if((pp>=p_i_R+p_i_A)&&(pp<p_i_R+p_i_A+p_d_R)){
      spatial_grid_R[rand_row,rand_col]<- spatial_grid_R[rand_row,rand_col]-1
    }
    #### death of resources
    if((pp>=p_i_R+p_i_A+p_d_R)&&(pp<p_i_R+p_i_A+p_d_R+p_d_A)){
      spatial_grid_A[rand_row,rand_col]<- spatial_grid_A[rand_row,rand_col]-1
    }
    #### formation of compund through encounter
    if((pp>=p_i_R+p_i_A+p_d_R+p_d_A)&&(pp<p_i_R+p_i_A+p_d_R+p_d_A+p_e)){
      spatial_grid_R[rand_row,rand_col]<- spatial_grid_R[rand_row,rand_col]-1
      spatial_grid_A[rand_row,rand_col]<- spatial_grid_A[rand_row,rand_col]-1
      spatial_grid_C[rand_row,rand_col]<- spatial_grid_C[rand_row,rand_col]+1
    }
    #### decomposition or handling
    if((pp>=p_i_R+p_i_A+p_d_R+p_d_A+p_e)&&(pp<p_i_R+p_i_A+p_d_R+p_d_A+p_e+p_d)){
      spatial_grid_C[rand_row,rand_col]<- spatial_grid_C[rand_row,rand_col]-1
      spatial_grid_A[rand_row,rand_col]<- spatial_grid_C[rand_row,rand_col]+2
    }
    #### movement of resources
    if((pp>=p_i_R+p_i_A+p_d_R+p_d_A+p_e+p_d)&&(pp<p_i_R+p_i_A+p_d_R+p_d_A+p_e+p_m_R)){
    ...
    if((pp>=p_i_R+p_i_A+p_d_R+p_d_A+p_e+p_d+p_m_R)){
...
mean_field <- function(t,state,parms) {
  with(as.list(c(state,parms)),{
    ...
    dA <- lambda_A + 2*nu*C - delta_A*A - (alpha/V)*R*A
    dC <- (alpha/V)*R*A - nu*C
    return(list(c(dR,dA,dC)))
  })
}

time  <- seq(0,max_time,delta_t)
state <- c(R=N_R,A=N_A,C=N_C)
...
colnames(det) <- c("time","pop_R","pop_A","pop_C")
...
steady_state <- function(parms){
  ...
  all_results       <- c(DELTA_R, DELTA_A, DELTA_A2,const_1,const_2,const_3,
                         R_1_R,R_2_R,A_1_R,A_2_R,C_1_R,C_2_R,
                         R_1_A,R_2_A,A_1_A,A_2_A,C_1_A,C_2_A)
...
max_pop <- max(max_pop_R,max_pop_A,max_pop_C)
...
mtext(paste("PARAMETERS\n",
            ...
            " Alpha/V = ",alpha,"/",V," = ",alpha/V," Nu = ", signif(nu,digits=3),"\n",
            sep=""),
