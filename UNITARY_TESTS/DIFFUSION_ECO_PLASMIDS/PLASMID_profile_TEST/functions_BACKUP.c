...
{ 
  /* During donor-recipient conjugation, a plasmid set from the donor is randomly selected from its 
     ...
     This assert verifies if the given conjugation plasmid set from the donor, this is, 
     a given subset from the whole donor plasmid profile, effectively changes the plasmid profile of 
     the recipient. This requires: 

     1. Permitted conjugation between (bacteria) strain types (conjugation condition)
     2. The conjugation set (as a whole) selected from the donor should be able to infect the recipient 
        bacterial type (full infection condition).
     3. The conjugation set (as a whole) selected from the donor should be totally compatible with all  
...
{
  /* Input: 
     . n, label of subpopulation or local variable index (true index of the local variable)
...
void Setting_Reactive_Recipient_Donor_Pairs_and_Rates(Parameter_Table * Table) 
{
  ...
  ///   to pass the full infection conditiion or the full compatibility condition or the complementarity    
  ///   condition.
  int N;
  int k, l, n; 
