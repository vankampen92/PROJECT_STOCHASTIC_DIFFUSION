						                                          ...
						                                          double Value)
{
  ...
  if( Table->No_of_CELLS > 1 ) { 
    ...
    /* Number of Cells to be chosen */
    No_of_CHOICES = (int)(0.05 * Table->No_of_CELLS);
    ...
    for (k = 0; k<No_of_CHOICES; k++){    
      ...
      for(n=0; n<Table->LOCAL_STATE_VARIABLES; n++) {
        /* Flies (2) and Workers (0) scattered at random 
    ...
    Table->Vector_Model_Variables_Time_0[1] = 0.0;  /* Initial value for queens */
