...
int function (double t, const double y[], double dydt[], void *params)
{
  ...
  for (j=0; j<Table->No_of_CELLS; j++) {
    ...
    dydt[A]  = -Table->Alpha_C_0 *y_R/K_R *y[A] + Table->Nu_C_0*y[RA] -Table->Chi_C_0* y[RA]/K_R *y[A] +Table->Eta_C_0 *y[ARA];

    dydt[RA] = -Table->Nu_C_0*y[RA] + Table->Alpha_C_0 *y_R/K_R *y[A] -Table->Chi_C_0* y[RA]/K_R *y[A] +Table->Eta_C_0 *y[ARA];
