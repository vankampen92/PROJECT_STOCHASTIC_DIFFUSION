...
int function (double t, const double y[], double dydt[], void *params)
{
  ...
  /* K_Q: Total Max No of Potential Nesting Trees in the Local Patch */ 
  ...
  for (j=0; j<Table->No_of_CELLS; j++) {
    ...
    /* Lambda_R_0 represents an exteral passive
       ...
       patches.
    */
    dydt[W]  = Table->Lambda_R_0 -Table->Delta_R_0 *y[W] +Table->Beta_R *y[Q] -Table->Alpha_C_0*y[W]/K_Q *y[F];             
    
    dydt[Q]  = Table->Lambda_R_1 +Table->Eta_R *(K_Q-y[Q])/K_Q *y[W] -Table->Delta_R_1*y[Q]; 
    
    dydt[F]  = Table->Lambda_C_0 +Table->Nu_C_0 *y[WF] -Table->Delta_C_0 *y[F] ;

    dydt[WF] = Table->Alpha_C_0*y[W]/K_Q *y[F] -Table->Delta_C_1*y[WF] -Table->Nu_C_0*y[WF] ;
