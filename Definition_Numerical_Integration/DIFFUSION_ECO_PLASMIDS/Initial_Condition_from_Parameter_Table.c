						                                          ...
						                                          double Value)
{
  ...
  if( Table->No_of_CELLS > 1 ) { 
    for (J=0; J<Table->No_of_CELLS; J++) {
      ...
      Sp = 0;                     /* Species 1 (or any other bacterial type) */ 
      if (N_X%2 == 0 && N_Y%2 == 0) {
    ...
    No_of_CHOICES = 10; /* Number of types to be chosen */

    for (k = 0; k<No_of_CHOICES; k++){ 
      Sp = (int)(Table->No_of_RESOURCES * gsl_rng_uniform(r)); 

      Table->Vector_Model_Variables_Time_0[Sp] = Value; /* Initial value for resource propagules (Sp 0)      */
    }

    Table->Vector_Model_Variables_Time_0[0] = Value; /* Initial value for population of type 0)              */

    assert(Table->No_of_RESOURCES > No_of_CHOICES);
							                                               ...
							                                               double Value_1)
{
  ...
  for(j=0; j<Table->No_of_CELLS; j++) {
    ...
    for(k=0; k<Table->No_of_RESOURCES; k++) {
      ...
      if( Sp == k ) {
        Table->Vector_Model_Variables_Time_0[R]  = Value_0; /* Initial value for resources Species Sp  */
      } 
      if( Sp_1 == k ) {
								  ...
								  double Value)
{
  ...
  for (J=0; J<Table->No_of_CELLS; J++) {
    for(Sp = 0; Sp < Table->No_of_ RESOURCES; Sp++) {
