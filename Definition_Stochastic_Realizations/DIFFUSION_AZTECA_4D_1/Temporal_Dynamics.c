...
void Temporal_Dynamics(Community ** My_Community, Parameter_Table * Table, Stochastic_Rate * Rate)
{
  ...
  */
  ...
  /* K_Q: Total Max No of Potential Nesting Tress (per local patch) */ 
  ...
  for(i=0; i<No_of_CELLS; i++){
    ...
    /* 3: Queen Out-Migration (P --> P-1) and some other patch gains one */ 
    OutMigration = P->Total_Per_Capita_Out_Migration_Rate[Q];
    assert( 4 * Table->Lambda_C_1 ==  OutMigration ); 
        
    P->rate[n] = OutMigration;                            P->rToI[n]  = OutMigration * (double)P->n[Q]; 
    P->ratePatch += P->rToI[n];
    #if defined BINARY_TREE_SUPER_OPTIMIZATION
      Table->Leaves[GRAND_No_of_EVENTS++]->value = P->rToI[n];
    #endif
    ...
    /* 4: Queen External Immigration event */
    P->rate[n] = Table->Lambda_R_1;                        P->rToI[n]  = Table->Lambda_R_1; 
    P->ratePatch += P->rToI[n]; 
    #if defined BINARY_TREE_SUPER_OPTIMIZATION
      Table->Leaves[GRAND_No_of_EVENTS++]->value = P->rToI[n];
    #endif
    ...
    /* 6: Worker Production by Queens */
    P->rate[n] = Table->Beta_R ;                           P->rToI[n] = P->rate[n] * (double)P->n[Q];
    P->ratePatch += P->rToI[n];
    #if defined BINARY_TREE_SUPER_OPTIMIZATION
      Table->Leaves[GRAND_No_of_EVENTS++]->value = P->rToI[n];
    #endif
    ...
    /* 7: New Queen production by Queens (conducive to nest establishment)  */
    if(P->n[Q] > 0) 
      P->rate[n] = Table->Eta_R * (K_Q-(double)P->n[Q])/K_Q * (double)P->n[W]/(double)P->n[Q];
    else if(P->n[Q] == 0)
      P->rate[n] = 0.0;
    else{
      printf("Something very wrong is happening when calculating total rates\n");
      printf("(at Temporal_Dynamics.c). The program will exit\n");
      exit(0); 
    }
                                                            P->rToI[n] = P->rate[n] * (double)P->n[Q];
    P->ratePatch += P->rToI[n];
    #if defined BINARY_TREE_SUPER_OPTIMIZATION
      Table->Leaves[GRAND_No_of_EVENTS++]->value = P->rToI[n];
    #endif
    ...
    /* 8: Fly Out-Migration (F ---> F-1) and some other patch gains one */ 
    ...
    /* 9: Fly External Immigration (F ---> F + 1) */  
    ...
    /* 11: Attack: Fly Consumption of a Worker Item and 'Dimmer' Formation */
    P->rate[n]= Table->Alpha_C_0*(double)P->n[W]/K_Q;      P->rToI[n]= P->rate[n]*(double)P->n[F];
    P->ratePatch += P->rToI[n];
    #if defined BINARY_TREE_SUPER_OPTIMIZATION
      Table->Leaves[GRAND_No_of_EVENTS++]->value = P->rToI[n];
    #endif
    ...
    /* 12: Fly larval development (within Worker heads) into adult flies  */ 
