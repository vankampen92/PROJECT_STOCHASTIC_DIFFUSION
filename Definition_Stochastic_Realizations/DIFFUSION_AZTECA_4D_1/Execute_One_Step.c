		                  ...
		                  int * Event, int * x_Patch)
{
  ...
  */
  ...
  switch( n_Event )
    ...
    case  3:  /* Out-Migration (Q --> Q - 1) from patch x to some other patch */         /* Outmigraiton (W) */
      Positivity_Control( 3, Table, x, Q, Y[Q], J[Q] );
      Y[Q]--; J[Q]--; Patch->n[Table->Q]--;

      y = Other_Patch_Population_Increase(3, x, Q, Table->Q, Table);
      /* If immigration fails, the loss of a queen in the source patch 
    ...
    case  4:  /* External Immigration from outside the system (Q --> Q + 1) */             /* External Imm W */
    ...
    case  5:  /* Local Death of Queens (Q --> Q - 1)  */                                 /* Death (W) */
      Positivity_Control( 5, Table, x, Q, Y[Q], J[Q] );
      Y[Q]--; J[Q]--;  Patch->n[Table->Q]--;

      break;
    
    case  6:  /* Worker Production by Queens (W --> W + 1) */                            /* Production (W) */
      Y[W]++; J[W]++;  Patch->n[Table->W]++;

      break;

    case  7:  /* New queen production  (Q ---> Q + 1 )  */ 
      Boundness_Control_Local_Patch( 7, Table, x, Q, Y[Q], J[Q] );
      Y[Q]++;  J[Q]++;   Patch->n[Table->Q]++;

      break;

    case  8:  /* Out-Migration (F --> F-1) from patch x and some other patch 
    ...
    case  9:  /* External Immigration from outside the system (F --> F+1) */             /* External Imm (F) */
    ...
    case  10:  /* Local Death  (F --> F-1)  */                                              /* Death (F) */
      Positivity_Control( 10, Table, x, F, Y[F], J[F] );
      Y[F]--; J[F]--;  Patch->n[Table->F]--;

      break;

    case 11: /* Attack: Fly attack on workers */ /* W + F --->  WF + F */
      Positivity_Control( 11, Table, x, W, Y[W], J[W] );
      Y[W]--; J[W]--; Patch->n[Table->W]--;
      
      Y[WF]++; J[WF]++;  Patch->n[Table->WF]++;
      break;

    case 12: /* Fly Laval Development into Adult Flies */  /* WF ---> F */
      Positivity_Control( 12, Table, x, WF, Y[WF], J[WF] );
      Y[WF]--; J[WF]--;  Patch->n[Table->WF]--;
    ...
    case 13: /* Local Death of Parasitized Workers (WF)   /* WF  ---> WF - 1  */
      Positivity_Control( 13, Table, x, WF, Y[WF], J[WF] );
      Y[WF]--; J[WF]--;  Patch->n[Table->WF]--;
					                          ...
					                          Parameter_Table * Table)
{
  ...
  */
  ...
  /* n_Q (number of queens) has to be always lower or equal to K_Q */
  if( Sp == 1 ) {
			                 ...
			                 int x_out, int x_in, int Sp, int jS, double Y, int J)
{
  ...
  */
  ...
  double K_Q = (double)Table->K_R;                              /* -HK      */
                                   ...
                                   int x, int jS, double Y, int J)
{
  ...
  double K; 
    ...
    /* K_Q: Total Max No of Potential Nesting Trees (per local area) */ 
    double K_Q = (double)Table->K_R;                         /* -HK  */
