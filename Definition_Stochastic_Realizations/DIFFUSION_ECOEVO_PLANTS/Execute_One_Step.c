		                  ...
		                  int * Event, int * x_Patch)
{
  ...
  int Sp_Out;
  /* Index of the Species the focal species mutates into */
  /* It is intialized with an impossible FLAG            */
  /* because 0 <= Sp_Out <= Table->No_of_RESOURCES-1     */
  Sp_Out = Table->No_of_RESOURCES; 
  ...
  */
  ...
  RP  = x*Table->LOCAL_STATE_VARIABLES + 2*Sp;
  R   = x*Table->LOCAL_STATE_VARIABLES + 2*Sp+1;
  ...
  switch( n_Event_Sp )
    ...
    case  0:  /* Out-Migration (RP --> RP - 1) from patch x to some other patch */         /* Outmigraiton (RP) */
      ...
      Y[RP]--; J[RP]--; Patch->n[2*Sp + Table->RP]--;

      y = Some_Other_Patch_Population_Increase(x, 2*Sp+Table->RP, Table);
    ...
    case  4:  /* Propagules Establishment (R ---> R+1 and RP ---> RP-1)  */                /* Establishment (RP) */
      ...
      Y[RP]--; J[RP]--;  Patch->n[2*Sp + Table->RP]--;    
      Y[R]++;  J[R]++;   Patch->n[2*Sp + Table->R]++;
                         Patch->m_0--;   /* Free space gets reduced */

      break;

    case 5:   /* Propagules Establshment (OUT-Mutation)  (RP ---> RP - 1)*/
      Positivity_Control( 5, Table, x, RP, Y[RP], J[RP] );

      Y[RP]--; J[RP]--;  Patch->n[2*Sp + Table->RP]--;

      Sp_Out = Some_Other_Species_Establishes(x, Sp, Table);                               /* Sp mutates into Sp_Out */

      Patch->m_0--;   /* Free space gets reduced */

      break; 

    case 6:   /* Propagules Establshment (IN-Mutation)                   */
                                                                                            /* Other Species mutates into Sp */
      /* The local propensity of an incoming mutant from the extanding species Sp is 0 !!!
         (unless there is an external arrival of mutants for all the species into the system) */
      printf(" This is an impossible event... Event = %d. The program will exit", n_Event_Sp);
      exit(0); 

      break;

    case  7:  /* Death of Adult Resources (R ---> R-1)  */                                       /* Death (R) */
      Positivity_Control( 7, Table, x, R, Y[R], J[R] );

      Y[R]--; J[R]--;  Patch->n[2*Sp + Table->R]--;

      Patch->m_0++;   /* Free space gets increased */
  ...
  (*Event) = n_Event;  x_Patch[0] = x;  x_Patch[1] = y;  x_Patch[2] = Sp_Out; 
}

int Some_Other_Species_Establishes(int x, int Sp,
					                         Parameter_Table * Table)
{
  /* Input: 
     ...
     . Sp_Out: A new species arising from mutation from extant species Sp 
  */
  int R; 
  int Sp_Out;
  ...
  if(       Sp == 0)                            Sp_Out = 1;
  else if ( Sp == (Table->No_of_RESOURCES-1) )  Sp_Out = Sp-1; 
  else { /* Mutation Up and Down with Equal Probability */  
    if(RANDOM < 0.5) Sp_Out = Sp-1; 
    else             Sp_Out = Sp+1; 
  }
 
  R = x*Table->LOCAL_STATE_VARIABLES + 2*Sp_Out+1;  /* Adult Individual Plant of Sp_Out */
  Y[R]++;  J[R]++;   Patch[x]->n[2*Sp_Out + Table->R]++;
