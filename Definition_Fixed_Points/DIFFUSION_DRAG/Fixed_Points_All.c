...
{
  int i; 
  double q, q_Star, K_R, G_Star, B_Star; 
  ...
  if (Table->Lambda_C_0 == 0.0 && Table->Lambda_R_0 == 0.0 ) {  
    ...
    if ( Coexistence_Condition( Table ) == 0 && q > 0.0 ) {
      ...
      G_Star  = Table->Lambda_C_1*q_Star/(Table->Delta_C_1 - Table->Beta_C*q_Star) * K_R;

      B_Star  = Table->Beta_R + Table->p_1 * G_Star; 

      Vector_Stationarity_Lower[R] = (B_Star * q_Star - Table->Delta_R_0)/Table->Alpha_C_0 * K_R;
